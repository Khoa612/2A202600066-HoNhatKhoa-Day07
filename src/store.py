...
class EmbeddingStore:
    ...
    def add_documents(self, docs: list[Document]) -> None:
        ...
        Uses embed_batch() if the embedding function supports it (single API call
        for all docs). Falls back to sequential embedding for plain callables.
        """
        if not docs:
            return

        # Batch path: 1 API call for all documents
        if hasattr(self._embedding_fn, "embed_batch"):
            ...
            # Sequential fallback for plain callables (tests, lambdas)
            for doc in docs:
